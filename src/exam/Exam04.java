    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++){ // количество строк
            for (int j = 0; j < n; j++){ // количество столбцов
                ...
                else if ((j >= i + 1) && (j <= n - 1 - i - 1)) { // заполняем верхний треугольник одноразрядными целыми от 1 до 9
                    ar[i][j] = (int)(Math.random() * 9 + 1);
                    sum += ar[i][j];
                }
                else if ((j < i) && (j >= n - i)) { // заполняем нижний треугольник одноразрядными целыми от 1 до 9
                    ar[i][j] = (int)(Math.random() * 9 + 1);
                    sum += ar[i][j];
                }
                else if ((j <= i - 1) && (j < n - 1 - i)) { // заполняем левый треугольник одноразрядными целыми от -1 до -9
                    ar[i][j] = (int)(Math.random() * (-9) - 1);
                    sum += ar[i][j];
                }
                else if ((i >= n - j) && (i <= j - 1)) { // заполняем правый треугольник одноразрядными целыми от -1 до -9
                    ar[i][j] = (int)(Math.random() * (-9) - 1);
                    sum += ar[i][j];
                }
